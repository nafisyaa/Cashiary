...
// Ambil total saldo, total pemasukan, total pengeluaran
$totalPemasukan = 0;
$totalPengeluaran = 0;
...
<div class="container-fluid p-4">
  ...
  <div class="card p-3 mb-4">
    ...
    <table class="table">
      ...
      <tbody>
        <?php
        $query = mysqli_query($conn, "SELECT pengeluaran.*, kategori.nama_kategori FROM pengeluaran JOIN kategori ON pengeluaran.kategori_id = kategori.kategori_id ORDER BY tanggal DESC");
        while ($row = mysqli_fetch_assoc($query)) {
        ?>
        <tr>
          <td><?= date('d M Y', strtotime($row['tanggal'])) ?></td>
          <td><span class="badge bg-danger"><?= $row['nama_kategori'] ?></span></td>
          <td><?= $row['deskripsi'] ?></td>
          <td class="text-danger">-Rp <?= number_format($row['jumlah'], 0, ',', '.') ?></td>
          <td>
            <a href="edit_pengeluaran.php?id=<?= $row['id'] ?>&from=pengeluaran" class="text-primary me-2"><i class="bi bi-pencil-square"></i></a>
            <a href="hapus_pengeluaran.php?id=<?= $row['id'] ?>&from=pengeluaran" class="text-danger" onclick="return confirm('Yakin ingin menghapus?')"><i class="bi bi-trash-fill"></i></a>
          </td>
        </tr>
        <?php } ?>
      </tbody>
